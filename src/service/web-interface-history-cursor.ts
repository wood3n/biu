 ...
 */
export interface WebInterfaceHistoryCursorParams {
  /** 历史记录截止目标 id，默认为 0 */
  max?: number;
  /** 历史记录截止目标业务类型，默认为空 */
  business?: HistoryBusinessType;
  /** 历史记录截止时间时间戳，默认为 0（当前时间） */
  view_at?: number;
  /** 历史记录分类筛选，默认为 all */
...
/** data 结构 */
export interface WebInterfaceHistoryCursorData {
  cursor: HistoryCursorInfo;
  tab: HistoryTabItem[];
  list: HistoryListItem[];
}

/** 顶层响应结构 */
export interface WebInterfaceHistoryCursorResponse {
  ...
  ttl: number;
  /** 历史记录数据 */
  data: WebInterfaceHistoryCursorData;
}

/**
 * 获取历史记录列表(Web端)
 * - 认证：Cookie (SESSDATA)，已在 axios 配置中开启 withCredentials
 * - IFS（无限滚动）：使用 max / business / view_at 作为下一页游标
 */
