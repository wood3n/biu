...
import { RiDeleteBinLine, RiExternalLinkLine, RiPlayFill } from "@remixicon/react";
...
const ListItem = ({ data, isActive, onPressItem, hideCover, className, titleClassName }: Props) => {
  ...
  const handleAction = (key: string) => {
    if (key === "bililink") openBiliVideoLink(data);
    if (key === "del") delPage(data.id);
  };

  return (
    <ContextMenu items={items} onAction={handleAction} className="h-full">
      <Button
        as="div"
        key={data.id}
        fullWidth
        ...
        className={twMerge(
          "group flex h-full w-full min-w-auto items-center justify-between rounded-md p-2",
          className,
        )}
      >
        <div className="m-0 flex min-w-0 flex-1 items-center">
          ...
          <div className={clx("flex min-w-0 flex-auto flex-col items-start space-y-1", { "ml-2": !hideCover })}>
            <MarqueeText
              className={twMerge("text-base", isActive ? "text-primary" : "text-foreground", titleClassName)}
              active={isActive}
            >
